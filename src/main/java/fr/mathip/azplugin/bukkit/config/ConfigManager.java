    ...
    public void applyUIComponents(Player player) {
        for (PacketUiComponent uiComponent : UIComponents) {
            ...
            if (!uiComponent.getHoverText().isEmpty()) {
                azChatComponent.setHoverEvent(new AZChatComponent.HoverEvent("show_text",
                        uiComponent.getHoverText().replaceAll("%player%", player.getName())));
            }
            if (!uiComponent.getCommmand().isEmpty()) {
                azChatComponent.setClickEvent(new AZChatComponent.ClickEvent("run_command",
                        uiComponent.getCommmand().replaceAll("%player%", player.getName())));
            }
